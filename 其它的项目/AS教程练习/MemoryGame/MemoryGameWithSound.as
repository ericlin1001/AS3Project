package {
	...
	//
	public class MemoryGameWithSound extends Sprite {
		...
		//constrator
		public function MemoryGameWithSound() {
			//
		...
		public function nextLevel():void {
			...
			setLevelText();
			//
			lightOnTimer=new Timer(520,level+1);
		...
		private function lightOn(e:TimerEvent):void {
			if (pLight>=level) {//flashing light is over;
				...
				lights[playOrders[pLight]].flashOn();
				pLight++;
			}
		}
				...
				if (light!=null) {
					if (light.num==playOrders[prLight]) {
						...
						if (prLight>=level-1) {
							...
							Mouse.hide();
							dispatchEvent(new GameEvent(GameEvent.NEXTLEVEL,null));
						}
					} else {
						lose();
						trace(playOrders);
		...
		public function getPlayOrders():Array{
			return playOrders;
		}
