package
{
	...
	public class Player extends FlxSprite
	{
		...
		protected static const PLAYER_WATER_MULTIPLYER:Number = 0.1;
		...
		protected static const UNDERWATER_COLOUR:uint = 0x8F88FF;
		...
		public override function update():void
		{
			...
			if (inWater)
			{
				this.color = UNDERWATER_COLOUR;
				
				if (FlxG.keys.UP)
					acceleration.y = -drag.x * PLAYER_WATER_MULTIPLYER;
				else if (FlxG.keys.DOWN)
					acceleration.y = drag.x * PLAYER_WATER_MULTIPLYER;
				else
					acceleration.y = GRAVITY_ACCELERATION * PLAYER_WATER_MULTIPLYER;
			...
			if(FlxG.keys.justPressed("C"))
			{
				...
				else if(aimingDown)
				{
					bY += height - 4;
					bYVel = BULLET_VELOCITY;
					if (!inWater) velocity.y -= BULLET_BOOST;
				}
